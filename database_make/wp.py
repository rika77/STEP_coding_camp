   ...
   def generate(self):
       ...
       for wiki_article in self.collection.get_all_documents():
           ...
           for node in parser.parse(wiki_article._text, as_nodes=True):
               ...
               if node.is_nor():
                   features = node.feature.split(',')
                   # 10000回に1回self.db.commit()する
                   # node重複を消す必要がある
                   if features[0] == '名詞':
                       j+=1
                       s.add((node.surface, wiki_article.title,))
                       ...
                       # # c = self.db.cursor()
                       # # row = c.execute("SELECT text, opening_text, auxiliary_text, categories, headings, wiki_text, popularity_score, num_incoming_links FROM articles WHERE title=?", (doc_id,)).fetchone()
                       # insert_sql = "INSERT into postings (term, document_id) values (?, ?)"
                       # # 10setくらい入るかな？
                       # content.append((node.surface, wiki_article.title,))
                       # k += 1
                       # # まあ最後の方見過ごすが、とりあえず...
                       # if k >= 10:
                       #     c.executemany(insert_sql, content)
                       #     k = 0
                       #     content = []
                       #
